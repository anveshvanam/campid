...
import { faEdit, faFileEdit, faGreaterThan, faLessThan, faSearch } from '@fortawesome/free-solid-svg-icons'
                ...
                <div className='profileContent'>
                    {/* <Grid container>
                        <Grid item container sm={6} >
                            <Grid item sm={5} className='profilePicDiv'>
                            ...
                            <Grid item sm={7} className='profileName'>
                                <h1>{user.city}</h1>
                                <h2>{user.email}</h2>
                                <h3>Elite Sports Camps</h3>
                            </Grid>
                        </Grid>
                        <Grid item sm={6} className='password_account'>
                    ...
                    <Grid container marginTop={{ xl: '40px', md: '20px', xs: '10px' }} gap={'10px'}>
                        <Grid item className='details'><p>Details</p></Grid>
                    ...
                    <Grid container spacing={3} marginTop={{ xl: '35px', md: '20px', xs: '10px' }}>
                        ...
                        <Grid item sm={3} xs={12}>
                            ...
                            {!edit ? <p>{user.faxNumber}</p> :
                                <input type='input' value={user.faxNumber} onChange={change} name='faxNumber' id='faxNumber' />}

                        </Grid>
                    </Grid>
                    <Grid container spacing={2} marginTop={{ xl: '25px', md: '15px', xs: '10px' }}>
                        <Grid item sm={3} xs={12}><label>Camp</label>
                            {!edit ? <p>{user.city}</p> :
                                <input type='input' value={user.city} onChange={change} name='city' id='city' />}
                        </Grid>

                        <Grid item sm={3} xs={12}><label>Role</label>

                            {!edit ? <p>Director</p> :
                                <input type='input' value='Director' />}
                        </Grid>
                        <Grid item sm={3} xs={12}><label>Website</label>
                            {!edit ? <p>www.elitesports.com</p> :
                                <input type='input' value='www.elitesports.com' />}
                        </Grid>
                    </Grid>
